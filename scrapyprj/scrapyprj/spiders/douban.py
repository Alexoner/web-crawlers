...
class DoubanSpider(scrapy.Spider):
    ...
    def parse_nextAndSub(self,response):
    	...
    	for i in range(1,len(timeList)+1):
    		...
    		if self.monthStr == intMonth and self.dayStr == intDay:
    			...
    			param['user_id'] = re.search(r'people/([\s\S]*)/',authorUrl).group(1)
    			param['reply_count'] = replyNum
    			param['latest_time'] = timeStr
    			yield scrapy.Request(url, headers = self.header, callback = parse_detail,meta = param)
    		else:
                    continue
    	    except Exception as e:
                print e
    ...
    def parse_detail(self,response):
    	#处理帖子主体内容
    	pic_urls = safe_extract(response.xpath("//div[contains(@class,'topic-figure')]/img/@src"))
	...
	replyUserList = safe_extract(response.xpath("//ul[@class='topic-reply']/li//h4/a/text()"))
	...
	for i in range(1,len(replyUserList)+1):
	    ...
	    #提取回复时间
	    repTime = safe_extract(response.xpath("//ul[@class='topic-reply']/li["+str(i)+"]//h4/span/text()"))
	    commentEntity['reply_time'] = repTime
	    commentEntity['id'] = hashlib.md5(topicId+repTime).hexdigest()
	    commentEntity['outer_id'] = id
	    commentEntity['user_url'] = safe_extract(response.xpath("//ul[@class='topic-reply']/li["+str(i)+"]//h4/a/@href"))
	    commentEntity['user_name'] = userName
	    replyContent = safe_extract(response.xpath("//ul[@class='topic-reply']/li["+str(i)+"]//div[contains(@class,'reply-doc')]/p/text()"))
