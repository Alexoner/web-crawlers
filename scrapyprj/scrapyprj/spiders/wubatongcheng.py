...
class WubatongchengSpider(scrapy.Spider):
    ...
    def start_requests(self):
        listMap = getProvince_City(
            '/Users/xueliang.xl/web-crawlers/scrapyprj/scrapyprj/province_city_name_hlj.txt')
        for provinceJson in listMap:
            ...
            for cityJson in cityList:
                ...
                if cityCode in self.existList:
                    self.logger.info('city %s already done' % cityName)
                else:
    ...
    def parse_nextAndSub(self, response):
        ...
        if (urlList and len(urlList) > 2):
            ...
            num += 1
            nextUrl = 'http://' + \
                response.meta['cityCode'] + '.58.com/xiaoqu/pn_' + str(num)
        ...
        for i in range(1, len(blockList) + 1):
            url = safe_extract(
                response.xpath(
                    "//table[@class='tbimg']/tbody/tr[" +
                    str(i) +
                    "]//ul/li[@class='tli1']/a/@href"))
            second_num = safe_extract(
                response.xpath(
                    "//table[@class='tbimg']/tbody/tr[" +
                    str(i) +
                    "]//ul/li[@class='tli3'][1]/span[1]/text()"))
            rend_num = safe_extract(
                response.xpath(
                    "//table[@class='tbimg']/tbody/tr[" +
                    str(i) +
                    "]//ul/li[@class='tli3'][1]/span[1]/text()"))
   ...
   # 解析detail
    def parse_detail(self, response):
        ...
        alias_name = safe_extract(response.xpath(
            "//h1[@class='xiaoquh1']/span/text()"))
        if alias_name:
        ...
        if locationStr:
            ...
            result['longitude'] = re.search(
                r'lat:\'(.*?)\'', locationStr).group(1)
            result['latitude'] = re.search(
                r'lon:\'(.*?)\'', locationStr).group(1)
        # 区和小区
        area = ''
        subarea = ''
        areaList = safe_extract(response.xpath(
            "//dl[@class='bhrInfo']/dd[2]//a[contains(@href,'xiaoqu')]/text()"))
        if areaList:
            for i in range(0, len(areaList)):
                ...
                if i == 1:
                    ...
                    break
        # 地址和邮编
        address = safe_extract(response.xpath(
            "//span[@class='ddinfo']/a[contains(@href,'xiaoqu')]/parent::span/text()"))
        # 小区邮编的unicode编码
        postcode = safe_extract(response.xpath(
            u"//dd/span[contains(text(),'小区邮编')]/parent::dd/text()"))
        result['address'] = trim(address)  # 小区地址
        result['post_code'] = postcode  # 邮政编码
        # 小区标签
        tagList = safe_extract(response.xpath(
            "//div[@class='assessList']/span//text()"))
        if tagList:
            result['tags'] = tagList.replace('\t', ',')

        # 建筑年代和类型，unicode编码
        year_type = safe_extract(response.xpath(
            u"//dd/span[contains(text(),'建筑年代')]/parent::dd/text()"))
        year = ''
        # btype = ''
        if year_type:
        ...
        # 物业费
        wuyeMoney = safe_extract(response.xpath(
            u"//dd/span[contains(text(),'物业费')]/parent::dd/span[@class='ddinfo']/text()"))
        if wuyeMoney:
            wuyeMoney = wuyeMoney.split("/")[0]
            result['prop_price'] = trim(wuyeMoney.replace('元', ''))

        # 开发商、物业公司
        kaifashang = safe_extract(response.xpath(
            u"//dd/span[contains(text(),'开发商')]/parent::dd/text()"))
        wuyeCompany = safe_extract(response.xpath(
            u"//dd/span[contains(text(),'物业公司')]/parent::dd/text()"))
        if kaifashang:
        ...
        # 幼儿园、小学、大学
        kindergarden = safe_extract(response.xpath(
            u"//div[@class='peitaoDiv']/ul/li/span[contains(text(),'幼儿园')]/text()"))
        ...
        # #公交
        bus = safe_extract(response.xpath(
            u"//div[@id='peitao_2']/ul/li/span[contains(text(),'公')]/parent::li/span[@class='liinfo']/text()"))
        if bus:
            result['bus'] = bus
        # 地铁
        subway = safe_extract(response.xpath(
            u"//div[@id='peitao_2']/ul/li/span[contains(text(),'地')]/parent::li/span[@class='liinfo']/text()"))
        if subway:
            result['subway'] = subway

        # 配套信息的document
        peitaoDocument = safe_extract(
            response.xpath("//div[contains(@class,'peitaoInfo')]"))
